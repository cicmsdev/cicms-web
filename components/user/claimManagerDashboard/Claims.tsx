...
type Props = {
  ...
  onBuildPdfData?: () => PdfMeta | undefined;
};
...
export default function Claims({
  ...
  const handleExportPdfFrontend = async () => {
    ...
    if (meta.logoUrl) {
      try {
        const img = await toDataURL(meta.logoUrl);
        const w = 52;
        const h = 52;
    ...
    doc.setFontSize(10);
    if (meta.rangeText) {
      doc.text(meta.rangeText, 40, y);
      y += 14;
    }
    if (meta.extraNote) {
      doc.text(meta.extraNote, 40, y);
      y += 14;
    }
    ...
    const body = sortedClaims.map((c) => [
      String(c.id),
      c.projectName ?? "—",
      c.status ?? "—",
      c.incidentDate ? new Date(c.incidentDate).toLocaleDateString() : "—",
      c.claimTypeLabel ?? humanize(c.claimType ?? "OTHERS"),
      String(c.documents ?? 0),
      c.submitterName
        ? `${c.submitterName}${
            c.submitterEmail ? ` <${c.submitterEmail}>` : ""
          }`
        : "—",
      c.companyName ?? "—",
      c.evaluatorName
        ? `${c.evaluatorName}${
            c.evaluatorEmail ? ` <${c.evaluatorEmail}>` : ""
          }`
        : "—",
    ]);
    ...
    (autoTableMod as any).default(doc, {
      ...
      headStyles: { fillColor: [10, 32, 69], textColor: 255, fontSize: 10 },
      margin: { left: 40, right: 40 },
      theme: "striped",
      columnStyles: {
        0: { cellWidth: 90 }, // Claim ID
        1: { cellWidth: 70 }, // Project
        2: { cellWidth: 60 }, // Status
        3: { cellWidth: 50 }, // Incident
        4: { cellWidth: 90 }, // Type
        5: { cellWidth: 40, halign: "center" }, // Documents
        6: { cellWidth: 140 }, // Submitter
        7: { cellWidth: 80 }, // Company
        8: { cellWidth: 140 }, // Evaluator
      },
    });

    const fname = `claims_${new Date().toISOString().slice(0, 10)}.pdf`;
    doc.save(fname);
  };
    ...
    <>
      ...
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-3 mb-4">
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={handleExportPdfFrontend}
            className="px-3 py-2 text-sm rounded-lg border border-[#0a2045] text-white"
            style={{ backgroundColor: "#0a2045" }}
