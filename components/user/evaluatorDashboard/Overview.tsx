...
export default function Overview({ claims, total, onSelectClaim }: Props) {
  // Filter out the statuses we don't want to show
  const filteredStatuses = UI_STATUSES.filter(
    status => status !== "Approved" && status !== "Submitted" && status !== "Rejected"
  );

  const statusCounts = filteredStatuses.map((status) => ({
    label: status,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        ...
        {/* Status cards (only showing In Review, In Court, and Resolved) */}
      ...
      <div className="bg-white rounded-xl shadow p-6">
        ...
        <ul className="space-y-3 text-sm text-gray-700">
          <li className="flex items-center gap-2">
            ...
            Claim C-1012 was resolved yesterday.
          </li>
          <li className="flex items-center gap-2">
